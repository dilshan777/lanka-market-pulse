...
function SearchContent({ articles, categories }: Props) {
  ...
  useEffect(() => {
    ...
    if (dateFrom) {
      filtered = filtered.filter((a) => new Date(a.publishedAt) >= new Date(dateFrom));
    }
    if (dateTo) {
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        <div className="relative max-w-2xl">
          ...
          />
          {query && (
            <button onClick={() => setQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-muted">
      ...
      <div className="mb-6">
        ...
        >
          ...
          Filters
          {hasActiveFilters && (
            <span className="px-1.5 py-0.5 bg-lanka-accent text-lanka-dark text-[10px] font-bold rounded-full">Active</span>
          )}
        </button>
        ...
        {showFilters && (
          <div className="mt-4 p-4 bg-card border border-border rounded-xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="text-xs font-medium text-lanka-text-muted uppercase mb-2 block">Category</label>
                <select
              ...
              <div>
                <label className="text-xs font-medium text-lanka-text-muted uppercase mb-2 block">From Date</label>
              ...
              <div>
                <label className="text-xs font-medium text-lanka-text-muted uppercase mb-2 block">To Date</label>
                <div className="relative">
                  <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-lanka-text-muted" />
            ...
            {hasActiveFilters && (
              <button onClick={clearFilters} className="mt-4 text-sm text-lanka-accent hover:text-lanka-accent-dim transition-colors">
                Clear all filters
              </button>
            )}
          </div>
      ...
      <div className="mb-4">
        <p className="text-sm text-lanka-text-muted">
          Found <span className="font-medium text-lanka-text-primary">{results.length}</span> result{results.length !== 1 ? "s" : ""}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {results.length > 0 ? (
            results.map((article) => <ArticleCard key={article.slug} article={article} variant="default" />)
          ) : (
            <div className="text-center py-16 bg-card border border-border rounded-xl">
              <Search className="w-12 h-12 text-lanka-text-muted mx-auto mb-4" />
              <p className="text-lanka-text-muted">No results found</p>
            </div>
          )}
        </div>
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 rounded-lg text-sm transition-colors ${
                    selectedCategory === cat.slug ? "bg-lanka-accent/10 text-lanka-accent" : "text-lanka-text-secondary hover:bg-muted"
                  }`}
                >
                  <span>{cat.name}</span>
