...
import { FileText, Download, Calendar, Clock, ArrowRight, Loader2 } from "lucide-react";
...
export default function DailyReportsPage() {
  ...
  const handleGeneratePDF = async () => {
    ...
    setGenerating(false);
    alert("PDF generation would be handled by the API. In production, this downloads a temporary PDF.");
  };
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <section className="mb-8">
        <div className="bg-gradient-to-r from-lanka-accent/10 to-lanka-blue/10 border border-lanka-accent/20 rounded-xl p-6">
          <h2 className="text-lg font-bold text-lanka-text-primary mb-4">
          ...
          <p className="text-xs text-lanka-text-muted mt-3">
            PDFs are generated on-demand and not stored permanently. Files are automatically removed after download.
          </p>
        </div>
      </section>
      ...
      {briefing && (
        <section className="mb-8">
          <SectionHeader
            title="Today's AI Market Briefing"
            subtitle={formatDate(briefing.date)}
          />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <SectionHeader
            ...
            subtitle="Browse past market reports"
          />
          {reports.length > 0 ? (
            <div className="space-y-4">
              {reports.map((report) => (
                <div
                  key={report.id}
                  className="bg-card border border-border rounded-xl p-5 hover:border-lanka-accent/30 transition-colors"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                          report.type === "daily"
                            ...
                            : report.type === "weekly"
                              ? "bg-lanka-blue/10 text-lanka-blue"
                      ...
                      <h3 className="text-base font-semibold text-lanka-text-primary mb-2">
                        {report.title}
                      </h3>
                      <p className="text-sm text-lanka-text-secondary line-clamp-2">
                        {report.summary}
                      </p>
                    </div>
                    <button
        ...
        <aside className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <div className="space-y-3">
              <ReportTypeCard
                type="Daily"
                description="End-of-day market summary with key highlights"
              ...
              <ReportTypeCard
                type="Monthly"
                description="In-depth monthly economic and market report"
  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
      ...
      <div>
        ...
        <div className="text-xs text-lanka-text-muted">{description}</div>
      </div>
    </div>
  );
}
